...
public class AdminStaffDetailsController {
    ...
    public void initialize() {
        ...
        loadTable();
        tblUsers.setPlaceholder(new Label("No staff records found."));
    }
    ...
    public void loadTable() {
        ...
        try {
            List<UserDetailsDTO> staffDetails = userService.getAllStaffDetails();
            allUsersList.addAll(staffDetails);
            filteredUsersList.addAll(staffDetails);
        } catch (SQLException e) {
            showAlert(Alert.AlertType.ERROR, "Database Error", "Failed to load staff from database.");
        }
    }
    ...
    private void handleUpdate() {
        ...
        if (selectedUser == null) {
            showAlert(Alert.AlertType.WARNING, "No Selection", "Please select a staff member from the table to update.");
            return;
        }
        ...
        confirmAlert.setHeaderText("Update Staff Information");
        confirmAlert.setContentText("Are you sure you want to update: " + selectedUser.getName() + "?");
        Optional<ButtonType> result = confirmAlert.showAndWait();
        if (result.isPresent() && result.get() == ButtonType.OK) {
            updateStaffInService();
        }
    }

    private void updateStaffInService() {
        try {
            ...
            int updatedRows = userService.updateStaff(updatedUser);
            if (updatedRows > 0) {
                showAlert(Alert.AlertType.INFORMATION, "Success", "Staff information updated successfully.");
                loadTable();
                handleClear(null);
            } else {
                showAlert(Alert.AlertType.ERROR, "Error", "Failed to update staff information.");
            }
        } catch (Exception e) {
            showAlert(Alert.AlertType.ERROR, "Error", "An unexpected error occurred while updating staff.");
        }
    }
    ...
    private void handleDelete() {
        UserDetailsDTO selectedUser = tblUsers.getSelectionModel().getSelectedItem();
        if (selectedUser == null) {
            showAlert(Alert.AlertType.WARNING, "No Selection", "Please select a staff member from the table to delete.");
            return;
        }

        Alert confirmAlert = new Alert(Alert.AlertType.CONFIRMATION);
        confirmAlert.setTitle("Confirm Deletion");
        confirmAlert.setHeaderText("Delete Staff");
        confirmAlert.setContentText("Are you sure you want to delete: " + selectedUser.getName() + "?");
        Optional<ButtonType> result = confirmAlert.showAndWait();
        if (result.isPresent() && result.get() == ButtonType.OK) {
            deleteStaffFromService(selectedUser);
        }
    }

    private void deleteStaffFromService(UserDetailsDTO user) {
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Error", "An unexpected error occurred while deleting staff.");
        }
    }
